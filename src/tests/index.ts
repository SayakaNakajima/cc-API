...
import Gryph from "../entities/glyph";
...
describe("expense manager", () => {
  ...
  let app: Application;
  let glyphRepo: Repository<Glyph>;
  ...
  before(async () => {
    ...
    glyphRepo = getRepository(Glyph);
  });

  after(async () => {
    await glyphRepo.delete({ id: Not(IsNull()) });
  });
  ...
  beforeEach(async () => {
     ...
     * password: tester
     */
    let testGryph = new Glyph();
    testGryph.glyph = "研";
    testGryph.radical = "石";
    testGryph.strokes = 9;
    testGryph.test_grade = "８級";
    testGryph = await glyphRepo.save(testGryph);
  ...
  describe("Auth and user services", () => {
    it("should restrict access by unauthenticated user", async () => {
      const res = await chai.request(app).get(`/glyphs/${TEST_USER_ID}`);
      expect(res).to.have.status(401);
    });


    it("should be able to retrieve a user", async () => {
      const res = await chai.request(app).get(`/glyphs/${TEST_USER_ID}`).set("Authorization", accessToken);
      ...
      // Ensure password-related fields are inaccessible by glyphs
    ...
    it("should be able to remove a user", async () => {
      ...
      // Delete
      const deleteResponse = await chai.request(app).delete(`/glyphs/${TEST_USER_ID}`).set("Authorization", accessToken);
      expect(deleteResponse).to.have.status(200);

      // Try to retrieve after delete
      const getResponse = await chai.request(app).get(`/glyphs/${TEST_USER_ID}`).set("Authorization", accessToken);
  ...
  describe("Account service", () => {
    ...
    it.skip("should restrict access by unauthenticated user", async () => {
       ...
       *
       * Advanced requirements
       * - Make sure unauthenticated glyphs are not allowed to
    ...
    it.skip("should restrict access by unauthorised user", async () => {
       ...
       *
       * Advanced requirements
       * - Make sure unauthorised glyphs are not allowed to access others' accounts
       */
    });
